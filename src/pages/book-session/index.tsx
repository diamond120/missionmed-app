...
const BookSession = ({ addUpcomingSession, title, moduleType, timezone, credit }) => {
        ...
        {userRole === 'tutor' ? (
          <Radio key={student.id} value={student.id} disabled={checkCredit(student)}>
            <div className={'avatar'}>
              <Avatar src={student.profile_picture} size={40} icon={<UserOutlined />} />
              <div className={'name-degree'}>
                <h4 className={'tutor-name'}>{student.full_name}</h4>
                {student.phone_number && <span>Ph No: {student.phone_number + ', '}</span>}
                {student.country && <span>Country: {student.country + ', '}</span>}
                <span>
                  Credit:
          ...
          <Radio key={tutor.id} value={tutor.id}>
            <div className={'avatar'}>
              <Avatar src={tutor.profile_picture} size={40} icon={<UserOutlined />} />
              <div className={'name-degree'}>
                ...
                >
                  <span>
                    {tutor.university &&
                      tutor.university.map((item, index) => (
                        <span key={index}>
                          {item.school}({item.degree}){tutor.university.length - 1 != index && ','}
                        </span>
                      ))}
                  </span>
                </div>
  ...
  const TutorCollapse = memo(function TutorCollapse({ value = null, onChange, tutors, students }) {
    return (
      <>
        {userRole !== 'tutor' ? (
          <Radio.Group onChange={onChange} value={value}>
            <Collapse bordered={false} defaultActiveKey={['1']} expandIconPosition={`end`} className='site-collapse-custom-collapse'>
              {tutors &&
                tutors.map((tutor) => (
                  <Panel header={<TutorPanelHeader tutor={tutor} />} key={tutor.id} className='site-collapse-custom-panel'>
    ...
    return (
      <>
        <div className={'choose-tutor'}>
          ...
          <h4 className={'sub-title'} style={{ padding: '0 10px' }}>
            We recommend you pick someone you haven’t sat a mock with before.
          </h4>
          {userRole === 'tutor' ? (
      ...
      <>
        <div className={'book-time-cal'}>
          <Calender
            ...
            prev={prev}
          />
          {userRole == 'tutor' && timezone != students.find((student) => student.id == form.getFieldValue('tutorId'))?.timezone && (
            <Alert style={{ top: 23 }} message='Note: Timings in Calendar are displaying based on Student Timezone.' showIcon />
          )}
        </div>
      </>
    )
  ...
  const Step4From = ({ form }) => {
    ...
    const tutorName =
      user.role == 'student'
        ? tutors.find((tutor) => tutor.id == formData.tutorId)?.full_name
        : students.find((student) => student.id == formData.studentId)?.full_name
      ...
      <>
        <div className={'session-details'} style={{ padding: '0 10px' }}>
          ...
          <div style={{ marginBottom: 21 }}>
            <h4 style={{ marginBottom: 0, fontSize: 14, fontWeight: 600 }}>{user.role == 'tutor' ? 'Student' : 'Tutor'}</h4>
        ...
        >
          <Radio.Group onChange={handleRadioChange}>
            ...
            <Tooltip title={recurringAvailable ? 'Already recurring session is booked by another sutdernt.' : ''}>
              <Radio value='Recurring Session' disabled={recurringAvailable}>
                Recurring Session{' '}
                {recurringAvailable && (
                  <>
        ...
        {showDropdown && (
          <>
            <Form.Item style={{ marginTop: '17px', marginBottom: '0px' }} label='Frequency' name='frequency'>
              <Select
        ...
        >
          <TextArea
            rows={3}
            placeholder='Note down questions, content, topics etc. that you’d like to focus on so your tutor know ahead of time...'
            style={{ fontSize: 16 }}
          />
        </Form.Item>
  ...
  const Step5From = ({ form }) => {
    ...
    return (
      <>
        <div className={'session-details'} style={{ padding: '0 10px' }}>
          <h3 style={{ fontSize: 16, color: '#312D42', fontWeight: '600' }}>Credit Card Details</h3>
          <Cards cvc={cvc} expiry={expiry} name={userName} number={number} focused={focused} callback={handleCallback} />
          <Form.Item
            style={{ marginTop: '17px', marginBottom: '0px' }}
            label='Name'
            name={'userName'}
          ...
          <Form.Item name={'issuer'} initialValue={issuer}>
  ...
  return (
    <>
      {!timezone ? (
        <Tooltip title={'Please select your timezone first.'} color={'#465078'}>
