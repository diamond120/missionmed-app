...
const FinalReviewUpload: FC<UploadStepProps> = ({ getUrlFile, id, }) => {
  ...
  const sendNotification =()=>{
    ...
    //     variables: {
    //       input: {
    //         action: "has uploaded final review for",
    //         is_read: false,
    //         application: id,
    //         is_from_tutor: true,
    //         student: studentId
  ...
  const finalReviewFile = selectedApplication?.attributes?.final_review?.data?.attributes?.url || '';
  const finalReviewFileName = selectedApplication?.attributes?.final_review?.data?.attributes?.name
  useEffect(() => {
    setLinkFile(finalReviewFile)
  }, [linkFile]);
  return (
    <React.Fragment>
      {linkFile !== ""
        ...
        <div className={"init-draft-loaded-file"} onClick={()=>{setFileUrl(finalReviewFile)}}>
          ...
          <div className={"init-draft-loaded-preview-link"}>
            ...
            <p className={"init-draft-loaded-preview-name"} >{finalReviewFileName}</p>
          </div>
        </div>
...
export default FinalReviewUpload;