...
const StudentTeachingSession = () => {
  ...
  const getMockInterviewDetails = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        setUpcomingSessions(response.data?.data?.upcomingsessions ? response.data?.data?.upcomingsessions : [])
        setPastSessions(response.data?.data?.pastsessions ? response.data?.data?.pastsessions : [])
        setAgenda(response.data?.data?.upcomingInterview?.agenda ?? null)
        setFreezeSessions(response.data?.data?.freezesessions ? response.data?.data?.freezesessions : [])
        setTimeZone(response.data?.data?.studentTimezone ?? null)
        setCredit(
          response.data?.data?.teachingSessionCredit != '' && response.data?.data?.teachingSessionCredit != null
            ? response.data?.data?.teachingSessionCredit
            : 0
        )
      } else {
  ...
  const items = [
    {
      key: '1',
      label: <FreezeSession title='Freeze Session' moduleType='teaching' addFreezeSession={addFreezeSession} />
    },
    {
      key: '2',
      label: (
        <CancleSession
          ...
          cancleUpcomingSession={cancleUpSession}
          addUpcomingSession={upcomingInterview}
        />
      )
    }
  ]
          ...
          <div className='flex'>
            ...
            <div className={'d_flex_center'} style={{ gap: 15, display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
                ...
                <Space direction='vertical' className='dropdownIcon'>
                  <Space wrap>
                    <Dropdown placement='bottomLeft' menu={{ items }} overlayClassName='session-dropdown'>
                      <EllipsisOutlined
                        style={{ padding: 11, borderRadius: 8, border: '1px solid #00000026', backgroundColor: '#fff', cursor: 'pointer' }}
                      />
                    </Dropdown>
                  </Space>
                </Space>
              ...
              {(upcomingSessions.length > 0 || pastSessions.length > 0 || freezeSessions.length > 0) && (
                <BookSession
                  title='Book Extra Session'
                  addUpcomingSession={addUpcomingSession}
                  moduleType='teaching'
                  credit={credit}
                  timezone={timezone}
                />
              ...
              <Alert
                ...
                message={
                  <>
                    ...
                    <Link
                      href='https://missionmed.com.au/checkout_step/interview-private-checkout/'
                      title='Purchase Here .'
                      target='_blank'
                    ></Link>
                  </>
                  ...
                  <div className={'con-box-wrap'} style={{ textAlign: 'center' }}>
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      {credit != '0' && credit != null && credit != '' ? (
                        ...
                        <>
                          You are only a click away from the best Interview Teaching tutors in
                          <br />
