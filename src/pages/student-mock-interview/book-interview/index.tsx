...
const BookInterview = ({ addUpcomingSession, timezone }) => {
  ...
  const getUniversityTutorList = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const interviewList = response.data.data.mockinterview.mockinterview
          ? response.data.data.mockinterview.mockinterview.split(',')
          : []
  ...
  const stepsTitles = [
    'Specify Your Priorites',
    `Choose ${user.role === 'tutor' ? 'Student' : 'Tutor'}`,
    'Book Time for Interview',
    'Check Last Details'
  ]
        ...
        <Radio key={tutor.id} value={tutor.id} disabled={checkCredit(tutor)}>
          <div className={'avatar'}>
            ...
            <div className={'name-degree'}>
              ...
              {user.role == 'tutor' && (
                <>
                  <span>Ph No: {tutor.phone_number + ', '}</span>
                  {tutor.country && <span>Country: {tutor.country + ', '}</span>}
                  <span>Credit: {tutor.credit || 0}</span>
                </>
              )}
              <div
                style={{
  ...
  const TutorCollapse = memo(function TutorCollapse({ value = null, onChange, tutors }) {
    return (
    ...
    return (
      <>
        <div className={'book-time-cal'}>
          ...
          {user.role == 'tutor' && timezone != tutors.find((tutor) => tutor.id == form.getFieldValue('tutorId'))?.timezone && (
  ...
  const Step4From = ({ form }) => {
    ...
    const tutorName =
      user.role == 'tutor'
        ? tutors.find((tutor) => tutor.id == formData.studentId)?.full_name
    ...
    <>
      {!timezone ? (
        <Tooltip title={'Please select your timezone first.'} color={'#465078'}>
          <Button className={'primary-button'}>Book Interview</Button>
        </Tooltip>
      ) : (
