...
const SidebarMenu: React.FC = ({ className, callBack }) => {
    ...
    const [current, setCurrent] = useState(
        location.pathname === "/" || location.pathname === "" ? defaultUrl : location.pathname
    );
        ...
        try {
            ...
            if (response.data.success) {
                if (localStorage.getItem("jwt")) {
                    ...
                    if (
                        response.data.data.show_story_feature == 1 &&
                        response.data.data.allow_without_login == 1
                    ) {
                        setShowStory(true);
                    } else {
                        setShowStory(false);
    ...
    return (
        ...
        >
            <div className={"sidebar-menu-logo-wrap"}>
                {collapsed ? (
                    <SvgIcon className={"sidebar-menu-logo-small"} type={"logoSidebar"} />
                ) : (
            ...
            <Menu
                ...
                expandIcon={
                    collapsed ? (
                        <CaretRightOutlined style={{ color: "rgb(255,255,255,0.65" }} />
                    ) : (
                        <CaretDownOutlined style={{ color: "rgb(255,255,255,0.65)" }} />
                    )
                }
                mode={"inline"}
          ...
          <Menu.Item  key={"/tutor/ucat-session"} onClick={() =>{navigate('tutor/ucat-session')}}  className={"custom-profile-item"} icon={<ReadOutlined  style={{fontSize: "24px", }}  />} >
            UCAT Sessions  
                ...
                <>
                    ...
                    >
                        <Menu.Item
                            onClick={() => {
                                navigate(
                                    isStudent ? "student/mock-interview" : "tutor/mock-interview"
                                );
                            }}
                            ...
                            onClick={() => {
                                navigate(
                                    isStudent
                                        ...
                                        : "tutor/teaching-session"
                                );
                            }}
                            key={
                                isStudent ? "/student/teaching-session" : "/tutor/teaching-session"
                            }
                        >
                            {" "}
                            Teaching Session{" "}
                    ...
                    <SubMenu
                        ...
                        title={<>UCAT Sessions </>}
                        icon={
                            <ReadOutlined style={{ fontSize: "24px", color: "white" }} key={"7"} />
                        }
                    >
                ...
                <Menu.Item
                    ...
                    style={{ position: "fixed", bottom: "24px", width: "280px" }}
                    key={"14"}
